...
api.interceptors.response.use((response) => response, async (error) => {
  ...
  const refreshToken = localStorage.getItem("refresh_token"); if (!refreshToken) return Promise.reject(error);
  if (refreshing) { const token = await new Promise<string | null>(resolve => refreshQueue.push(resolve)); if (!token) return Promise.reject(error); original.headers.Authorization = `Bearer ${token}`; return api(original); }
  refreshing = true;
  try { const response = await axios.post(`${API_BASE_URL}/auth/refresh/`, { refresh: refreshToken }); const access = response.data.access as string; localStorage.setItem("access_token", access); if (response.data.refresh) localStorage.setItem("refresh_token", response.data.refresh as string); refreshQueue.forEach(resolve => resolve(access)); refreshQueue=[]; original.headers.Authorization=`Bearer ${access}`; return api(original); }
  catch (refreshError) { localStorage.removeItem("access_token"); localStorage.removeItem("refresh_token"); refreshQueue.forEach(resolve=>resolve(null)); refreshQueue=[]; return Promise.reject(refreshError); }
...
export const authApi = { login:async(email:string,password:string)=>{const response=await api.post("/auth/login/",{email,password});localStorage.setItem("access_token",response.data.access);localStorage.setItem("refresh_token",response.data.refresh);return response.data;},register:async(full_name:string,email:string,password:string,confirm_password:string)=>(await api.post("/auth/register/",{full_name,email,password,confirm_password})).data,me:async()=>(await api.get<User>("/accounts/me/")).data,updateMe:async(data:Partial<Pick<User,"full_name"|"phone">>)=>(await api.patch<User>("/accounts/me/",data)).data,logout:()=>{localStorage.removeItem("access_token");localStorage.removeItem("refresh_token");}};
...
export const bookingsApi={list:async()=>(await api.get("/bookings/")).data,create:async(data:{tour_slug:string;departure_date:string;travelers:TravelerUploadDraft[];contact_email:string;contact_phone:string;payment_type:"full_payment"|"down_payment";payment_method:"card"|"zelle"})=>{const form=new FormData();form.append("tour_slug",data.tour_slug);form.append("departure_date",data.departure_date);form.append("contact_email",data.contact_email);form.append("contact_phone",data.contact_phone);form.append("payment_type",data.payment_type);form.append("payment_method",data.payment_method);form.append("travelers",JSON.stringify(data.travelers.map(({passport_document,passport_photo,...traveler})=>traveler)));data.travelers.forEach((traveler,index)=>{if(traveler.passport_document)form.append(`traveler_${index}_passport_document`,traveler.passport_document);if(traveler.passport_photo)form.append(`traveler_${index}_passport_photo`,traveler.passport_photo);});return(await api.post("/bookings/",form)).data;},get:async(id:number)=>(await api.get(`/bookings/${id}/`)).data};
export const paymentsApi={createOrder:async(bookingId:number)=>(await api.post("/payments/create-order/",{bookingId})).data as {orderId:string;amount:number;currency:string;keyId:string}};
...
export type AdminTourPayload={destination:number;slug:string;title:string;duration_days:number;duration_nights:number;price:number;currency:string;rating:number;review_count:number;cover_image:string;halal_features:string[];summary:string;departure_city:string;images:Array<{url:string;alt:string}>;itinerary:Array<{day:number;title:string;description:string}>};
export type AdminSummary={bookings:{total:number;pending:number;confirmed:number;cancelled:number};customers:number;enquiries:number;payments:{total:number;captured:number;failed:number}};
...
export type JotFormSubmission={id:string;created_at:string;answers?:Record<string,{name?:string;text?:string;answer?:unknown}>;[key:string]:unknown};
export type AdminJotFormResponse={configured:boolean;form_id?:string;form_url?:string;count:number;message?:string;submissions:JotFormSubmission[]};
export const adminApi={
  ...
  jotformRegistrations:async()=>(await api.get<AdminJotFormResponse>("/admin/jotform-registrations/")).data,
