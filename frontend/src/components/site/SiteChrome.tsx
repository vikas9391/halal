...
export function SiteHeader() {
  ...
  const { user } = useAuth();
  return <header className="site-header"><Link href="/" className="brand" aria-label="Halal Trails home" onClick={() => setMenuOpen(false)}><span className="brand-mark"><Landmark size={18} /></span><span><b>HALAL TRAILS</b><small>TRAVEL GROUNDED</small></span></Link><nav className={menuOpen ? "main-nav main-nav--open" : "main-nav"} aria-label="Primary navigation">{NAV_LINKS.map(link => <Link key={link.href} href={link.href} onClick={() => setMenuOpen(false)} style={location === link.href ? { opacity: 1, fontWeight: 700 } : undefined}>{link.label}</Link>)}<Link href="/traveler" onClick={() => setMenuOpen(false)}>My bookings</Link></nav><div className="flex items-center gap-2"><Link className="header-cta" href={user ? "/profile" : "/login"}>{user ? "My account" : "Sign in / Register"} <ChevronRight size={15} /></Link><motion.button whileTap={{ scale: 0.9 }} className="menu-button" onClick={() => setMenuOpen(open => !open)} aria-label="Toggle navigation"><AnimatePresence mode="wait" initial={false}>{menuOpen ? <motion.span key="close" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: .15 }} style={{ display: "flex" }}><X /></motion.span> : <motion.span key="open" initial={{ rotate: 90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }} transition={{ duration: .15 }} style={{ display: "flex" }}><Menu /></motion.span>}</AnimatePresence></motion.button></div></header>;
}

export function SiteFooter() {
  const contact = useContactSettings();
  const contactAction = contactLinks(contact.phone, contact.whatsapp);
  return <footer><Link className="brand" href="/"><span className="brand-mark"><Landmark size={18} /></span><span><b>HALAL TRAILS</b><small>TRAVEL GROUNDED</small></span></Link><p>Thoughtfully curated journeys from DFW to the world.<br /><a href={`tel:${contactAction.telephone}`}><Phone size={13} style={{ verticalAlign: "-2px", marginRight: "4px" }} />{contact.phone}</a>{" · "}<a href={`mailto:${contact.email}`}><Mail size={13} style={{ verticalAlign: "-2px", marginRight: "4px" }} />{contact.email}</a></p><div className="flex gap-3"><Link href="/login">Customer login</Link><Link href="/admin">Admin</Link></div></footer>;
}

export function MobileBookBar({ href = "/register/thanksgiving-umrah-2026" }: { href?: string }) { return <Link className="mobile-book" href={href}>Reserve your spot <ChevronRight size={16} /></Link>; }
