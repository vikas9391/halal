...
class TravelerInputSerializer(serializers.Serializer):
    ...
    def validate_passport_document(self, value):
        if value.size > 10 * 1024 * 1024: raise serializers.ValidationError("Passport file must be 10 MB or smaller.")
        if value.content_type not in {"application/pdf", "image/jpeg", "image/png", "image/webp"}: raise serializers.ValidationError("Passport must be a PDF, JPG, PNG, or WebP file.")
        return value
    def validate_passport_photo(self, value):
        if value.size > 5 * 1024 * 1024: raise serializers.ValidationError("Passport photo must be 5 MB or smaller.")
        if value.content_type not in {"image/jpeg", "image/png", "image/webp"}: raise serializers.ValidationError("Passport photo must be a JPG, PNG, or WebP image.")
        return value
...
class BookingDetailSerializer(BookingSerializer):
    ...
    def get_traveler_details(self, obj):
        return [{"fullName": t.full_name, "passportNumber": t.passport_number, "dateOfBirth": t.date_of_birth, "passportStatus": t.passport_status, "mobilityAssistance": t.mobility_assistance} for t in obj.travelers.all()]
...
class BookingCreateSerializer(serializers.Serializer):
    ...
    def validate_tour_slug(self, value):
        if not Tour.objects.filter(slug=value).exists(): raise serializers.ValidationError("Tour not found.")
        return value
    def validate_travelers(self, value):
        if not value: raise serializers.ValidationError("At least one traveler is required.")
        return value
    def create(self, validated_data):
        request = self.context["request"]
        tour = Tour.objects.get(slug=validated_data["tour_slug"])
        travelers_data = validated_data.pop("travelers")
        booking = Booking.objects.create(user=request.user, tour=tour, departure_date=validated_data["departure_date"], contact_email=validated_data["contact_email"], contact_phone=validated_data["contact_phone"], payment_type=validated_data["payment_type"], payment_method=validated_data["payment_method"], total_price=tour.price * len(travelers_data), status="pending")
        Traveler.objects.bulk_create([Traveler(booking=booking, **traveler) for traveler in travelers_data])
        return booking
